...
export const actions = {
	updateSelf: async ({ request, platform, locals, cookies }: import('./$types').RequestEvent) => {
		if (!locals.user || locals.user.role !== 'superadmin') {
	...
	createSuperadmin: async ({ request, platform, locals }: import('./$types').RequestEvent) => {
		if (!locals.user || locals.user.role !== 'superadmin') {
			return fail(401, { error: 'Unauthorized' });
		}
	...
	deleteSuperadmin: async ({ request, platform, locals }: import('./$types').RequestEvent) => {
		if (!locals.user || locals.user.role !== 'superadmin') {
			return fail(401, { error: 'Unauthorized' });
		}
