...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    let data = $$props["data"];
    head("3fdugz", $$renderer2, ($$renderer3) => {
      $$renderer3.title(($$renderer4) => {
        $$renderer4.push(`<title>Peringkat Ujian: ${escape_html(data.type_name)} - Siswa</title>`);
      });
    });
    $$renderer2.push(`<div class="space-y-6 animate-in"><div class="mb-8"><a href="/siswa/papan-peringkat" class="inline-flex items-center text-sm text-slate-500 hover:text-slate-800 transition-colors mb-4"><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg> Kembali ke Kategori</a> <h1 class="text-3xl font-bold text-slate-800 tracking-tight">Peringkat Per Ujian</h1> <p class="text-slate-500 mt-1">Daftar peringkat untuk setiap ujian pada kategori <span class="font-bold text-slate-700">${escape_html(data.type_name)}</span>.</p></div> <section class="mb-10"><h2 class="text-xl font-bold text-slate-800 mb-4 flex items-center gap-2"><svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-primary-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"></path></svg> Daftar Ujian</h2> `);
    if (data.exams.length === 0) {
      ...
      for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
        ...
        $$renderer2.push(`<!--]--></div></div> <div class="mt-auto pt-5 border-t border-slate-100"><a${attr("href", `/siswa/papan-peringkat/${stringify(exam.id)}`)} class="btn btn-primary w-full justify-center">Lihat Peringkat</a></div></div>`);
      }
      $$renderer2.push(`<!--]--></div>`);
    }
