...
function _page($$renderer) {
  ...
  for (let idx = 0, $$length = each_array_8.length; idx < $$length; idx++) {
    ...
    if (activeTypeTab === qType.id) {
      $$renderer.push("<!--[0-->");
      $$renderer.push(`<div class="space-y-6 animate-in fade-in duration-300 max-w-4xl mx-auto"><div class="grid grid-cols-1 md:grid-cols-12 gap-5 p-5 md:p-6 bg-gradient-to-br from-indigo-50/70 via-white to-purple-50/40 rounded-2xl border border-indigo-100 shadow-xs items-center"><div class="md:col-span-5 space-y-2"><div class="flex items-center gap-2.5"><span class="text-3xl">${escape_html(qType.icon)}</span> <div><h3 class="text-lg font-bold text-slate-900">${escape_html(qType.name)}</h3> <span${attr_class(`inline-block px-2 py-0.5 rounded-full text-[11px] font-bold border mt-0.5 ${stringify(qType.badgeColor)}`)}>${escape_html(qType.badge)}</span></div></div> <p class="text-slate-600 text-xs md:text-sm leading-relaxed pt-1">${escape_html(qType.desc)}</p></div> <div class="md:col-span-7 bg-white/90 p-4 rounded-xl border border-slate-200/80 shadow-xs space-y-2"><h4 class="text-xs font-bold text-slate-700 uppercase tracking-wider flex items-center gap-1.5"><span>💡</span> <span>Petunjuk &amp; Cara Mengerjakan:</span></h4> <ul class="space-y-1.5"><!--[-->`);
      ...
      for (let $$index_8 = 0, $$length2 = each_array_9.length; $$index_8 < $$length2; $$index_8++) {
        ...
        $$renderer.push(`<li class="flex items-start gap-2 text-xs md:text-sm text-slate-600 leading-relaxed"><span class="w-1.5 h-1.5 rounded-full bg-indigo-500 shrink-0 mt-2"></span> <span>${html(tip)}</span></li>`);
      }
      $$renderer.push(`<!--]--></ul></div></div> <div class="bg-slate-50/80 p-4 md:p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4"><div class="flex items-center justify-between px-1"><div class="flex items-center gap-2 text-xs md:text-sm font-bold text-slate-800"><span>🎮</span> <span>Simulasi Interaktif: Coba Jawab Soal di Bawah Ini</span></div> <span class="text-[11px] md:text-xs font-medium text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded-full border border-emerald-200">✓ Coba Klik &amp; Interaksi</span></div> <div class="bg-white p-5 md:p-8 rounded-2xl border border-slate-200 shadow-xs">`);
      QuestionRenderer($$renderer, {
        question: qType.sampleQuestion,
        answer: demoAnswers[qType.id] || ""
      });
      $$renderer.push(`<!----></div></div> <div class="flex items-center justify-between pt-2"><button type="button" class="px-4 md:px-5 py-2.5 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 hover:border-slate-300 text-slate-700 font-semibold text-xs md:text-sm transition-all flex items-center gap-2 shadow-xs"><svg class="w-4 h-4 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg> <span>Tipe Sebelumnya</span></button> <div class="flex items-center gap-1.5"><span class="text-xs md:text-sm font-bold text-slate-600 bg-slate-100 px-3 py-1 rounded-full">Tipe Soal ${escape_html(idx + 1)} dari ${escape_html(questionTypes.length)}</span></div> <button type="button" class="px-4 md:px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-xs md:text-sm transition-all flex items-center gap-2 shadow-sm shadow-indigo-600/20"><span>Tipe Selanjutnya</span> <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg></button></div></div>`);
    } else {
      $$renderer.push("<!--[-1-->");
    }
    $$renderer.push(`<!--]-->`);
