    ...
    function $$render_inner($$renderer3) {
      ...
      if (data.mediaItems.length === 0) {
        ...
        for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
          ...
          $$renderer3.push(`<!--]--> <div class="absolute top-2 left-2 flex flex-col gap-1 items-start"><span class="badge badge-primary shadow-sm">${escape_html(item.media_type)}</span> <span${attr_class(`badge ${item.is_public ? "bg-emerald-100 text-emerald-700 border-emerald-200" : "bg-slate-100 text-slate-600 border-slate-200"} shadow-sm text-[10px] border`)}>${escape_html(item.is_public ? "🌐 Publik" : "🔒 Privat")}</span></div></div> <div${attr_class(`p-4 flex-1 flex flex-col ${item.question_id ? "" : "bg-amber-50/50"}`)}><div class="flex-1 space-y-2 mb-4">`);
          if (item.question_id) {
            $$renderer3.push("<!--[0-->");
