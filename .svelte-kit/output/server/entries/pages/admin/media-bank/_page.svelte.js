...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    function $$render_inner($$renderer3) {
      ...
      if (form?.success) {
        $$renderer3.push("<!--[0-->");
        $$renderer3.push(`<div class="alert alert-success mb-6"><svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg> <span>${escape_html(form.success)}</span></div>`);
      } else {
        $$renderer3.push("<!--[-1-->");
      }
      $$renderer3.push(`<!--]--> `);
      if (data.mediaItems.length === 0) {
        ...
        for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
          ...
          $$renderer3.push(`<!--]--> <div class="absolute top-2 left-2"><span class="badge badge-primary shadow-sm">${escape_html(item.media_type)}</span></div></div> <div${attr_class(`p-4 flex-1 flex flex-col ${item.question_id ? "" : "bg-amber-50/50"}`)}><div class="flex-1 space-y-2 mb-4">`);
          if (item.question_id) {
            $$renderer3.push("<!--[0-->");
