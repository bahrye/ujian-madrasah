  ...
  $$renderer.component(($$renderer2) => {
    ...
    if (data.leaderboard.length === 0) {
      ...
      for (let index = 0, $$length = each_array.length; index < $$length; index++) {
        ...
        $$renderer2.push(`<!--]--> <div><p class="font-bold text-slate-800">${escape_html(student.student_name)}</p> <p class="text-xs text-slate-500">${escape_html(student.class_name || "Tidak ada kelas")} • ${escape_html(student.exams_completed)} Ujian</p></div></div></td><td class="p-4 text-center text-slate-600 font-medium">${escape_html(student.total_points)}</td><td class="p-4 text-center"><span class="inline-block px-3 py-1 bg-green-100 text-green-700 font-bold rounded-lg">${escape_html(student.total_score)}</span></td><td class="p-4 text-center text-slate-600 font-medium">${escape_html(student.avg_score.toFixed(2))}</td></tr>`);
      }
