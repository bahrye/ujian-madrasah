    ...
    if (each_array.length !== 0) {
      ...
      for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
        ...
        if (exam.start_time || exam.end_time) {
          $$renderer2.push("<!--[0-->");
          $$renderer2.push(`<span class="truncate">${escape_html(exam.start_time ? (/* @__PURE__ */ new Date(exam.start_time.replace(" ", "T") + (exam.start_time.includes(" ") && !exam.start_time.includes("Z") ? "Z" : ""))).toLocaleString("id-ID", { dateStyle: "short", timeStyle: "short" }) : "-")} 
								s/d 
								${escape_html(exam.end_time ? (/* @__PURE__ */ new Date(exam.end_time.replace(" ", "T") + (exam.end_time.includes(" ") && !exam.end_time.includes("Z") ? "Z" : ""))).toLocaleString("id-ID", { dateStyle: "short", timeStyle: "short" }) : "-")}</span> `);
          if (isOutOfBounds) {
            $$renderer2.push("<!--[0-->");
