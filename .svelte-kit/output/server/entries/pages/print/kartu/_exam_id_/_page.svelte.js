  ...
  $$renderer.component(($$renderer2) => {
    ...
    for (let pageIndex = 0, $$length = each_array.length; pageIndex < $$length; pageIndex++) {
      ...
      for (let $$index_1 = 0, $$length2 = each_array_1.length; $$index_1 < $$length2; $$index_1++) {
        ...
        $$renderer2.push(`<!--]--></div> <div class="flex flex-col items-center flex-shrink-0"><img${attr("src", getQrCodeImageUrl("https://ujian-madrasah.pages.dev", 100))} alt="QR Website" class="w-12 h-12 object-contain mix-blend-multiply" title="Scan Link Website Ujian"/> <span class="text-[7px] text-slate-500 font-bold tracking-tighter mt-0.5">WEB UJIAN</span></div></div> <div class="p-4 flex-1 flex flex-col"><div class="text-center font-bold text-xs mb-3 pb-2 border-b border-slate-300 leading-snug break-words">${escape_html(exam.title)}</div> <div class="flex-1 flex gap-3"><div class="flex gap-2 items-start flex-shrink-0"><div class="w-20 flex flex-col items-center gap-1"><div class="w-20 h-24 bg-slate-200 border border-slate-400 flex items-center justify-center text-[10px] text-slate-400 font-semibold text-center leading-tight p-1">FOTO 3x4</div> <div class="text-[9px] font-mono font-bold bg-slate-100 px-1 py-0.5 border border-slate-300 rounded text-center w-full truncate"${attr("title", p.username)}>${escape_html(p.username)}</div></div> <div class="flex flex-col items-center justify-center"><img${attr("src", getQrCodeImageUrl(generateStudentQrData(p.login_username || p.username, p.login_password || p.nisn, p.qr_token), 160))} alt="QR Login" class="w-20 h-20 object-contain border border-slate-300 p-0.5 rounded bg-white shadow-sm mix-blend-multiply" title="Scan QR untuk Login Siswa"/> <span class="text-[7px] font-bold text-slate-600 mt-1 uppercase tracking-tight">QR LOGIN</span></div></div> <div class="flex-1 text-xs space-y-1.5"><div class="grid grid-cols-[80px_8px_1fr] items-baseline"><span class="text-slate-600">Nama</span> <span>:</span> <span class="font-bold uppercase truncate">${escape_html(p.student_name)}</span></div> <div class="grid grid-cols-[80px_8px_1fr] items-baseline"><span class="text-slate-600">No. Peserta</span> <span>:</span> <span class="font-mono font-bold">${escape_html(p.nomor_peserta || "-")}</span></div> <div class="grid grid-cols-[80px_8px_1fr] items-baseline"><span class="text-slate-600">NISN</span> <span>:</span> <span class="font-mono">${escape_html(p.nisn || "-")}</span></div> <div class="grid grid-cols-[80px_8px_1fr] items-baseline"><span class="text-slate-600">Kelas</span> <span>:</span> <span class="font-bold">${escape_html(p.class_name || "-")}</span></div> <div class="grid grid-cols-[80px_8px_1fr] items-baseline"><span class="text-slate-600">Ruang / Sesi</span> <span>:</span> <span class="font-semibold">${escape_html(p.room_name || "-")} / Sesi ${escape_html(p.session_number || 1)}</span></div> <!--[-->`);
        ...
        $$renderer2.push(`<!--]--> <div class="grid grid-cols-[80px_8px_1fr] items-baseline"><span class="text-slate-600">Password</span> <span>:</span> <span class="font-mono bg-slate-100 px-1 border border-slate-200 rounded">${escape_html(p.login_password || p.plain_password || p.nisn || "******")}</span></div></div></div> <div class="mt-2 pt-2 border-t border-slate-200 flex justify-between items-end text-[9px] text-slate-500"><div>Simpan kartu ini dengan baik selama ujian.</div> <div class="font-semibold text-slate-700 text-right"><p class="text-[9px] mb-5">Panitia Ujian,</p> <p class="text-[9px] font-bold border-b border-slate-800 inline-block px-2">${escape_html(data.committeeName || "......................................")}</p></div></div></div></div>`);
      }
      $$renderer2.push(`<!--]--></div>`);
    }
