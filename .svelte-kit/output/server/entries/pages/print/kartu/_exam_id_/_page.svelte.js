...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
      ...
      $$renderer2.push(`<!--]--></div> <div class="w-14 h-14"></div></div> <div class="p-4 flex-1 flex flex-col justify-between"><div><div class="text-center font-bold text-sm mb-4 pb-2 border-b border-slate-300">${escape_html(exam.exam_type_name || "Ujian")} - ${escape_html(exam.subject_name || "Umum")}</div> <table class="w-full text-sm"><tbody><tr><td class="py-1 w-24 font-medium text-slate-700">No. Peserta</td><td class="py-1 w-4 text-center">:</td><td class="py-1 font-bold">${escape_html(p.nisn)}</td></tr><tr><td class="py-1 font-medium text-slate-700">Nama</td><td class="py-1 text-center">:</td><td class="py-1 font-bold">${escape_html(p.student_name)}</td></tr><tr><td class="py-1 font-medium text-slate-700">Kelas / Ruang</td><td class="py-1 text-center">:</td><td class="py-1 font-bold">${escape_html(p.class_name || "-")} / ..........</td></tr><tr><td class="py-1 font-medium text-slate-700">Username</td><td class="py-1 text-center">:</td><td class="py-1 font-mono font-bold tracking-wider">${escape_html(p.nisn)}</td></tr><tr><td class="py-1 font-medium text-slate-700">Password</td><td class="py-1 text-center">:</td><td class="py-1 font-mono font-bold tracking-wider">${escape_html(p.nisn)}</td></tr></tbody></table></div> <div class="mt-4 flex justify-between items-end"><div class="w-[3cm] h-[4cm] border-2 border-slate-300 flex items-center justify-center bg-slate-50 text-slate-400 text-xs text-center p-2">Pas Foto<br/>3 x 4</div> <div class="text-center"><p class="text-xs mb-10">Panitia Ujian</p> <p class="text-xs font-bold border-b border-slate-800 inline-block px-4">......................................</p></div></div></div></div>`);
    }
    $$renderer2.push(`<!--]--></div> `);
