  ...
  $$renderer.component(($$renderer2) => {
    ...
    for (let roomIdx = 0, $$length = each_array_2.length; roomIdx < $$length; roomIdx++) {
      ...
      for (let sessionIdx = 0, $$length2 = each_array_3.length; sessionIdx < $$length2; sessionIdx++) {
        ...
        $$renderer2.push(`<div${attr_class(clsx(roomIdx > 0 || sessionIdx > 0 ? "break-before-page pt-4 print:pt-0" : ""), "svelte-5lujtk")}><div class="flex items-center justify-between gap-4 pb-2 relative"><img src="/kemenag.png" alt="Logo Kemenag" class="w-20 h-20 object-contain shrink-0"/> <div class="flex-1 text-center font-serif px-2"><h4 class="font-semibold text-sm uppercase tracking-wider text-black m-0 leading-tight">KEMENTERIAN AGAMA REPUBLIK INDONESIA</h4> <h3 class="font-bold text-xl uppercase tracking-wide text-black m-0 my-0.5">${escape_html(school?.name || "NAMA SEKOLAH")}</h3> `);
        if (school?.address) {
          $$renderer2.push("<!--[0-->");
