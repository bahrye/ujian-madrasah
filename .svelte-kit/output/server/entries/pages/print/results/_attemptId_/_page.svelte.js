...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<!--]--> <div class="bg-white p-3 rounded-lg border-2 border-indigo-200 text-center min-w-[90px] shadow-sm print:bg-white"><span class="text-[10px] font-bold uppercase text-slate-400 block">Nilai Akhir</span> <span${attr_class(`text-2xl font-black ${(attempt.score ?? 0) >= 70 ? "text-emerald-600" : "text-rose-600"}`)}>${escape_html(attempt.score != null ? attempt.score.toFixed(1) : "-")}</span> <span class="text-[10px] font-medium text-slate-500 block mt-0.5">Poin: ${escape_html(attempt.score ? Math.round(attempt.score / 100 * attempt.total_points) : 0)}/${escape_html(attempt.total_points)}</span></div></div></div></div> <div class="font-sans"><div class="flex items-center justify-between mb-2"><h3 class="font-bold text-xs uppercase text-slate-700 tracking-wider">Rincian Jawaban Soal (${escape_html(answers.length)} Soal)</h3></div> <div${attr_class(
      "columns-1 sm:columns-2 print:columns-2 gap-3 space-y-0"
    )}><!--[-->`);
    const each_array = ensure_array_like(answers);
    for (let i = 0, $$length = each_array.length; i < $$length; i++) {
      let ans = each_array[i];
      $$renderer2.push(`<div${attr_class(
        `break-inside-avoid inline-block w-full mb-3 bg-white rounded-lg border-2 p-3 text-xs shadow-sm print:shadow-none ${ans.score_given === ans.max_points ? "border-emerald-400 bg-emerald-50/10" : ans.score_given > 0 ? "border-amber-400 bg-amber-50/10" : "border-rose-400 bg-rose-50/10"}`,
        "svelte-pp9uw4"
      )}><div class="flex items-center justify-between border-b border-slate-100 pb-1.5 mb-2 gap-2"><div class="flex items-center gap-1.5 flex-wrap"><span class="font-bold text-xs bg-slate-100 text-slate-800 px-1.5 py-0.5 rounded">Soal #${escape_html(ans.question_number)}</span> <span class="text-[10px] font-medium text-slate-500 bg-slate-50 px-1.5 py-0.5 rounded border border-slate-200">${escape_html(QUESTION_TYPE_LABELS[ans.type])}</span> `);
      if (ans.is_doubted) {
        $$renderer2.push("<!--[0-->");
      ...
      $$renderer2.push(`<!--]--></div> <div class="font-bold text-xs whitespace-nowrap"><span${attr_class(ans.score_given === ans.max_points ? "text-emerald-600" : ans.score_given > 0 ? "text-amber-600" : "text-rose-600")}>${escape_html(ans.score_given ?? 0)}</span> <span class="text-slate-400 font-normal">/${escape_html(ans.max_points)} Poin</span></div></div> <div class="prose prose-xs max-w-none text-slate-800 mb-2.5 bg-slate-50 p-2 rounded border border-slate-200/60 font-serif leading-relaxed">${html(ans.question_text)}</div> <div class="space-y-2 mt-auto pt-1"><div${attr_class(`rounded p-2 border ${ans.score_given === ans.max_points ? "bg-emerald-50/50 border-emerald-200" : ans.score_given > 0 ? "bg-amber-50/50 border-amber-200" : "bg-rose-50/50 border-rose-200"}`)}><div class="flex items-center justify-between mb-1"><span class="font-bold text-[10px] uppercase tracking-wider text-slate-600">Jawaban Siswa:</span> <span${attr_class(`text-[10px] font-semibold ${ans.score_given === ans.max_points ? "text-emerald-700" : ans.score_given > 0 ? "text-amber-700" : "text-rose-700"}`)}>`);
    ...
    $$renderer2.push(`<!--]--></div></div> <div class="mt-4 pt-3 border-t border-slate-300 flex items-center justify-between text-[10px] text-slate-500 font-sans"><span>Dicetak otomatis dari Ujian Online Madrasah (${escape_html((/* @__PURE__ */ new Date()).toLocaleDateString("id-ID"))})</span> <span>Halaman Detail Hasil Jawaban — ${escape_html(attempt.student_name)} (${escape_html(attempt.exam_title)})</span></div></div></div>`);
