  ...
  $$renderer.component(($$renderer2) => {
    ...
    if (each_array_1.length !== 0) {
      ...
      for (let $$index_1 = 0, $$length = each_array_1.length; $$index_1 < $$length; $$index_1++) {
        ...
        if (exam.class_names) {
          $$renderer2.push("<!--[0-->");
          $$renderer2.push(`<div class="flex items-start gap-2 text-sm text-slate-600"><svg class="w-4 h-4 text-slate-400 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"${attr("d", ICONS.school)}></path></svg> <div class="flex flex-col"><span class="font-medium line-clamp-2"${attr("title", exam.class_names)}>${escape_html(exam.class_names)}</span> <span class="text-xs text-slate-500 mt-0.5">${escape_html(exam.participant_count || 0)} Siswa</span></div></div>`);
        } else {
          $$renderer2.push("<!--[-1-->");
        }
