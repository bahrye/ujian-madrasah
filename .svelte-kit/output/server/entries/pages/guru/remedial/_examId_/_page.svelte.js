import { h as head, k as attr, e as escape_html, c as stringify, i as ensure_array_like, j as attr_class, a as attr_style, f as bind_props } from "../../../../../chunks/index.js";
  ...
  $$renderer.component(($$renderer2) => {
    ...
    if (attempts.length === 0) {
      ...
      for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
        ...
        $$renderer2.push(`<tr class="hover:bg-slate-50"><td class="pl-5"><div class="font-bold text-slate-800">${escape_html(attempt.student_name)}</div> <div class="text-xs text-slate-500">${escape_html(attempt.class_name || "-")}</div></td><td class="text-xs font-mono text-slate-600">${escape_html(parseDate(attempt.start_time).toLocaleTimeString("id-ID"))}</td><td><span${attr_class(`badge ${stringify(ATTEMPT_STATUS_COLORS[attempt.status] || "badge-slate")}`)}>${escape_html(ATTEMPT_STATUS_LABELS[attempt.status] || attempt.status)}</span></td><td class="text-center">`);
