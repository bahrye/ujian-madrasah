import { h as head, i as attr, c as ensure_array_like, e as escape_html, d as attr_class, a as stringify, b as bind_props } from "../../../../chunks/index.js";
  ...
  $$renderer.component(($$renderer2) => {
    ...
    if (filteredExams.length === 0) {
      ...
      for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
        ...
        $$renderer2.push(`<tr class="hover:bg-slate-50/50 transition-colors"><td><div class="font-bold text-slate-800 mb-1">${escape_html(exam.title)}</div> <div class="text-xs text-slate-500">Mapel: ${escape_html(exam.subject_name || "-")} | Durasi: ${escape_html(exam.duration_minutes)}m</div></td><td><form method="POST" action="?/toggleActive" class="inline-block"><input type="hidden" name="id"${attr("value", exam.id)}/> <button type="submit"${attr_class(`badge ${exam.is_active ? "badge-success hover:bg-emerald-200" : "badge-slate hover:bg-slate-300"} transition-colors cursor-pointer`)} title="Klik untuk mengubah status">${escape_html(exam.is_active ? "Aktif" : "Nonaktif")}</button></form></td><td class="text-xs text-slate-600 space-y-1"><div>Mulai: ${escape_html(exam.start_time ? new Date(exam.start_time).toLocaleString("id-ID") : "-")}</div> <div>Akhir: ${escape_html(exam.end_time ? new Date(exam.end_time).toLocaleString("id-ID") : "-")}</div></td><td><div class="flex items-center gap-3"><span class="text-xs font-medium text-slate-600 bg-slate-100 px-2 py-1 rounded-md" title="Jumlah Soal">${escape_html(exam.question_count)} soal</span> <span class="text-xs font-medium text-slate-600 bg-slate-100 px-2 py-1 rounded-md" title="Jumlah Peserta">${escape_html(exam.participant_count)} peserta</span></div></td><td><div class="flex items-center justify-end gap-2"><a${attr("href", `/guru/remedial/${stringify(exam.id)}`)} class="btn-sm btn-ghost text-indigo-600 hover:bg-indigo-50" title="Kelola &amp; Monitor Ujian">Monitor &amp; Kelola</a> <button class="btn-sm btn-ghost text-slate-400 hover:text-indigo-600" title="Edit Ujian"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round"${attr("d", ICONS.edit)}></path></svg></button> `);
