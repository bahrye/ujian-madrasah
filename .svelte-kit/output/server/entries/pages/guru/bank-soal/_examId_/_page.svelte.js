import { f as bind_props, h as head, e as escape_html, k as attr, j as attr_class, i as ensure_array_like, c as stringify } from "../../../../../chunks/index.js";
...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    function $$render_inner($$renderer3) {
      ...
      $$renderer3.push(`<div class="space-y-6 animate-in"><div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4"><div class="flex items-start sm:items-center gap-3"><a href="/guru/bank-soal" class="btn-ghost btn-sm mt-1 sm:mt-0"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round"${attr("d", ICONS.chevronLeft)}></path></svg></a> <div><h1 class="text-2xl font-bold text-slate-800 leading-tight">${escape_html(exam.title)}</h1> <p class="text-sm text-slate-500 mt-1">${escape_html(exam.subject || "Umum")} · ${escape_html(questions.length)} soal</p></div></div> <div class="grid grid-cols-2 sm:flex sm:items-center gap-2 pl-12 sm:pl-0">`);
      ...
      if (selectedQuestionIds.size > 0) {
        $$renderer3.push("<!--[0-->");
        $$renderer3.push(`<div class="fixed bottom-6 left-1/2 -translate-x-1/2 bg-white rounded-2xl shadow-2xl border border-slate-200 p-4 flex items-center gap-6 z-50 animate-in slide-in-from-bottom-8"><div><div class="text-slate-800 font-bold">${escape_html(selectedQuestionIds.size)} soal terpilih</div> <div class="text-slate-500 text-sm">Hapus massal soal yang dipilih</div></div> `);
        ConfirmForm($$renderer3, {
          action: "?/deleteBulk",
          ...
          $$slots: {
            ...
            buttonContent: ($$renderer4) => {
              {
                $$renderer4.push(`<svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round"${attr("d", ICONS.trash)}></path></svg> Hapus Terpilih`);
              }
            }
          }
        });
