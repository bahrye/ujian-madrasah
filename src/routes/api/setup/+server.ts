	...
	try {
		...
		await db.batch([
			...
			db.prepare(`INSERT INTO exams (school_id, title, description, subject, duration_minutes, start_time, end_time, is_active, created_by)
				VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`)
				.bind(1, 'Ujian Tengah Semester - Matematika', 'UTS Matematika Kelas 9 Semester Ganjil', 'Matematika', 90,
					'2024-12-01T08:00:00Z', '2024-12-31T17:00:00Z', 1, 2), // created_by = 2 (admin1)
			
			// Ujian untuk Sekolah 2
			db.prepare(`INSERT INTO exams (school_id, title, description, subject, duration_minutes, start_time, end_time, is_active, created_by)
				VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`)
				.bind(2, 'Ujian Akhir Semester - Bahasa Arab', 'UAS Bahasa Arab Kelas 9 Semester Ganjil', 'Bahasa Arab', 60,
