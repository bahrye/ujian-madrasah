	...
	try {
		...
		await db.batch([
			db.prepare(`INSERT INTO exams (title, description, subject, duration_minutes, start_time, end_time, is_active, created_by)
				VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
				.bind('Ujian Tengah Semester - Matematika', 'UTS Matematika Kelas 9 Semester Ganjil', 'Matematika', 90,
					'2024-12-01T08:00:00Z', '2024-12-31T17:00:00Z', 1, 1),
			db.prepare(`INSERT INTO exams (title, description, subject, duration_minutes, start_time, end_time, is_active, created_by)
				VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
				.bind('Ujian Akhir Semester - Bahasa Arab', 'UAS Bahasa Arab Kelas 9 Semester Ganjil', 'Bahasa Arab', 60,
					'2024-12-15T08:00:00Z', '2024-12-31T17:00:00Z', 1, 1),
			db.prepare(`INSERT INTO exams (title, description, subject, duration_minutes, start_time, end_time, is_active, created_by)
				VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
				.bind('Latihan Soal - IPA', 'Latihan soal IPA untuk persiapan ujian', 'IPA', 45,
					'2024-11-01T08:00:00Z', '2025-12-31T17:00:00Z', 0, 1),
		]);

		// Buat contoh soal untuk Ujian Matematika (exam_id = 1)
		await db.batch([
			// Pilihan Ganda
			...
			db.prepare(`INSERT INTO questions (exam_id, type, question_text, question_number, points, options_json, correct_answer_json)
				VALUES (?, ?, ?, ?, ?, ?, ?)`)
				.bind(1, 'pilihan_ganda', 'Jika x + 5 = 12, maka nilai x adalah...', 2, 2,
					JSON.stringify(['5', '6', '7', '8']),
					'"C"'),
			db.prepare(`INSERT INTO questions (exam_id, type, question_text, question_number, points, options_json, correct_answer_json)
				VALUES (?, ?, ?, ?, ?, ?, ?)`)
				.bind(1, 'pilihan_ganda', 'Luas segitiga dengan alas 10 cm dan tinggi 6 cm adalah...', 3, 2,
			...
			db.prepare(`INSERT INTO questions (exam_id, type, question_text, question_number, points, options_json, correct_answer_json)
				VALUES (?, ?, ?, ?, ?, ?, ?)`)
				.bind(1, 'benar_salah', 'Bilangan prima adalah bilangan yang hanya bisa dibagi 1 dan dirinya sendiri.', 4, 1,
					JSON.stringify(['Benar', 'Salah']),
					'"Benar"'),
			db.prepare(`INSERT INTO questions (exam_id, type, question_text, question_number, points, options_json, correct_answer_json)
				VALUES (?, ?, ?, ?, ?, ?, ?)`)
				.bind(1, 'benar_salah', 'Hasil dari 7² adalah 48.', 5, 1,
					JSON.stringify(['Benar', 'Salah']),
					'"Salah"'),
			...
			db.prepare(`INSERT INTO questions (exam_id, type, question_text, question_number, points, options_json, correct_answer_json)
				VALUES (?, ?, ?, ?, ?, ?, ?)`)
				.bind(1, 'isian_singkat', 'Berapakah hasil dari √144?', 6, 2, null, '"12"'),
			db.prepare(`INSERT INTO questions (exam_id, type, question_text, question_number, points, options_json, correct_answer_json)
				VALUES (?, ?, ?, ?, ?, ?, ?)`)
				.bind(1, 'isian_singkat', 'Nilai π (pi) dibulatkan menjadi 2 angka desimal adalah...', 7, 2, null, '"3.14"'),

			// Menjodohkan
			db.prepare(`INSERT INTO questions (exam_id, type, question_text, question_number, points, options_json, correct_answer_json)
				VALUES (?, ?, ?, ?, ?, ?, ?)`)
				.bind(1, 'menjodohkan', 'Jodohkan bangun datar berikut dengan rumus luasnya:', 8, 3,
					JSON.stringify({
			...
			db.prepare(`INSERT INTO questions (exam_id, type, question_text, question_number, points, options_json, correct_answer_json)
				VALUES (?, ?, ?, ?, ?, ?, ?)`)
				.bind(1, 'essay', 'Jelaskan perbedaan antara bilangan rasional dan bilangan irasional. Berikan masing-masing 2 contoh!', 9, 5, null, null),
			db.prepare(`INSERT INTO questions (exam_id, type, question_text, question_number, points, options_json, correct_answer_json)
				VALUES (?, ?, ?, ?, ?, ?, ?)`)
				.bind(1, 'essay', 'Sebuah taman berbentuk persegi panjang dengan panjang 20 m dan lebar 15 m. Di dalam taman terdapat kolam berbentuk lingkaran dengan jari-jari 5 m. Hitunglah luas taman yang tidak tertutup kolam!', 10, 5, null, null),
		]);

		// Buat contoh soal untuk Ujian Bahasa Arab (exam_id = 2)
		await db.batch([
			db.prepare(`INSERT INTO questions (exam_id, type, question_text, question_number, points, options_json, correct_answer_json)
				VALUES (?, ?, ?, ?, ?, ?, ?)`)
			...
			db.prepare(`INSERT INTO questions (exam_id, type, question_text, question_number, points, options_json, correct_answer_json)
				VALUES (?, ?, ?, ?, ?, ?, ?)`)
				.bind(2, 'pilihan_ganda', 'Isim yang menunjukkan tempat disebut...', 2, 2,
					JSON.stringify(['Isim Fa\'il', 'Isim Maf\'ul', 'Isim Makan', 'Isim Zaman']),
					'"C"'),
			db.prepare(`INSERT INTO questions (exam_id, type, question_text, question_number, points, options_json, correct_answer_json)
				VALUES (?, ?, ?, ?, ?, ?, ?)`)
				.bind(2, 'benar_salah', 'Fi\'il Madhi adalah kata kerja bentuk lampau.', 3, 1,
					JSON.stringify(['Benar', 'Salah']),
					'"Benar"'),
			db.prepare(`INSERT INTO questions (exam_id, type, question_text, question_number, points, options_json, correct_answer_json)
				VALUES (?, ?, ?, ?, ?, ?, ?)`)
				.bind(2, 'isian_singkat', 'Tuliskan arti dari "مدرسة" dalam Bahasa Indonesia.', 4, 2, null, '"sekolah"'),
			db.prepare(`INSERT INTO questions (exam_id, type, question_text, question_number, points, options_json, correct_answer_json)
				VALUES (?, ?, ?, ?, ?, ?, ?)`)
				.bind(2, 'essay', 'Buatlah 3 kalimat sederhana dalam Bahasa Arab menggunakan Mubtada dan Khobar!', 5, 5, null, null),
		]);

		// Buat contoh token
		await db.prepare(`INSERT INTO tokens (exam_id, token_code, is_released, created_by, expires_at)
			VALUES (?, ?, ?, ?, ?)`)
			.bind(1, 'MTK2024', 1, 4, '2025-12-31T23:59:59Z')
			.run();

		await db.prepare(`INSERT INTO tokens (exam_id, token_code, is_released, created_by, expires_at)
			VALUES (?, ?, ?, ?, ?)`)
			.bind(2, 'ARB2024', 0, 4, '2025-12-31T23:59:59Z')
			.run();
