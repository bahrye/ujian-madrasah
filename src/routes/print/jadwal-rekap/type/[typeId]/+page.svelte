<script lang="ts">
	...
	$: classList = (data.classList || []) as any[];
	...
	function buildClassScheduleData(schedules: any[]) {
...
{#each classList as item, cIdx}
	{@const { days, proctorMap, totalExams } = buildClassScheduleData(item.schedules || [])}
	<div 
		class="p-4 md:p-8 print:p-0 print:m-0 mx-auto bg-white transition-all {cIdx > 0 ? 'break-before-page mt-8 print:mt-0' : ''} {orientation === 'landscape' ? 'max-w-[29.7cm] mode-landscape' : 'max-w-[21cm] mode-portrait'}" 
	...
	>
		...
		<div class="text-center mb-6 font-serif">
			...
			{#if item.classData}
				<p class="text-sm font-bold uppercase mt-1">KELAS: {item.classData.name}</p>
			{/if}
		</div>
		...
		{#if days.length === 0}
			<div class="p-12 text-center text-slate-500 italic border border-black">
				Belum ada jadwal ujian untuk kelas ini.
			</div>
		{:else}
			<div class="w-full">
