...
<svelte:head>
	<title>Cetak Semua Daftar Hadir - {examType.name}</title>
</svelte:head>
...
<div class="no-print p-4 bg-slate-800 text-white border-b border-slate-700 flex flex-wrap items-center justify-between gap-4 sticky top-0 z-50 shadow-md">
	<div class="text-xs text-slate-300">
		Gunakan kertas <strong>F4 / Folio</strong> saat mencetak daftar hadir. Total {examDataList.length} Mata Ujian.
	</div>
...
<div class="p-4 md:p-8 print:p-0 print:m-0 max-w-[215.9mm] mx-auto bg-white" style="font-family: 'Times New Roman', Times, Arial, serif; font-variant-numeric: lining-nums tabular-nums;">
	...
	{#each examDataList as item, examIdx}
		...
		{#each Object.entries(participantsGrouped) as [roomName, sessionsDict], roomIdx}
			{#each Object.entries(sessionsDict as Record<number, any[]>) as [sessionNumStr, students], sessionIdx}
				...
				{@const classNames = Array.from(new Set(students.map((s: any) => s.class_name).filter(Boolean))).join(', ') || '-'}

				<div class={examIdx > 0 || roomIdx > 0 || sessionIdx > 0 ? "break-before-page pt-4 print:pt-0" : ""}>
					<!-- Kop Surat -->
