				...
				{#each data.examTypes as type}
					<div class="bg-gradient-to-br from-indigo-500 to-primary-600 rounded-2xl p-6 shadow-md text-white flex flex-col hover:shadow-lg transition-shadow">
						...
						<div class="mt-auto space-y-3">
							<a href="/siswa/papan-peringkat/type/{type.id}" class="bg-white text-primary-600 hover:bg-slate-50 font-semibold px-4 py-2.5 rounded-xl w-full text-center inline-block transition-colors shadow-sm text-sm">
								Peringkat Akumulasi
							</a>
							<a href="/siswa/papan-peringkat/type/{type.id}/exams" class="bg-white/20 text-white hover:bg-white/30 border border-white/30 font-semibold px-4 py-2.5 rounded-xl w-full text-center inline-block transition-colors text-sm">
								Peringkat Per Ujian
							</a>
						</div>
					</div>
