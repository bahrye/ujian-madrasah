...
<div class="px-5 py-6 sm:px-8 max-w-6xl mx-auto">
	<div class="mb-8">
		...
		<p class="text-slate-500 mt-1">Lihat peringkat siswa untuk ujian Anda.</p>
	</div>
	...
	<section class="mb-10">
		...
		{#if data.exams.length === 0}
			<div class="bg-white border border-slate-200 rounded-2xl p-8 text-center text-slate-500 shadow-sm">
				<p>Belum ada ujian Anda yang tersedia untuk dilihat peringkatnya.</p>
			</div>
		{:else}
				...
				{#each data.exams as exam}
					<div class="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm flex flex-col hover:shadow-md transition-shadow">
						...
						<div class="mt-auto pt-5 border-t border-slate-100">
							<a href="/guru/papan-peringkat/{exam.id}" class="btn btn-primary w-full justify-center">
								Lihat Peringkat
							</a>
						</div>
					</div>
			...
			<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
				{#each data.examTypes as type}
					<div class="bg-gradient-to-br from-indigo-500 to-primary-600 rounded-2xl p-6 shadow-md text-white flex flex-col hover:shadow-lg transition-shadow">
						...
						<div class="mt-auto">
							<a href="/guru/papan-peringkat/type/{type.id}" class="bg-white text-primary-600 hover:bg-slate-50 font-semibold px-4 py-2.5 rounded-xl w-full text-center inline-block transition-colors shadow-sm">
