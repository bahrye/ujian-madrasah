		...
		<div class="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
			<div class="flex items-center gap-3 flex-1 min-w-0">
				...
				<div class="min-w-0">
					...
					<p class="text-xs text-indigo-200 truncate">Mode Pratinjau Guru</p>
				</div>
			</div>
			
			<a href={$page.url.searchParams.get('from') === 'bank' ? '/guru/bank-soal' : `/guru/bank-soal/${exam.id}`} class="btn-sm bg-white/10 hover:bg-white/20 text-white border-0 flex-shrink-0">
		...
		{:else}
			<div class="flex items-center justify-center h-64">
				<div class="text-center bg-white p-8 rounded-2xl shadow-sm border border-slate-100 max-w-md w-full">
					...
					<a href={$page.url.searchParams.get('from') === 'bank' ? '/guru/bank-soal' : `/guru/bank-soal/${exam.id}`} class="btn btn-primary w-full">Kembali ke Bank Soal</a>
				</div>
			</div>
		{/if}
	</main>
