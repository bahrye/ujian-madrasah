...
<div class="space-y-6 animate-in">
	<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
		<div class="flex items-start sm:items-center gap-3">
			<a href={$page.url.searchParams.get('from') === 'bank' ? '/admin/bank-soal' : `/admin/exams/${exam.id}`} class="btn-ghost btn-sm mt-1 sm:mt-0">
		...
		<div class="card p-6 border-2 border-indigo-200 animate-in">
			...
			<form method="POST" action="?/create" use:enhance={() => { return async ({ update }) => { await update(); }; }} class="space-y-4">
				...
				<div>
					...
					<textarea id="q-text" name="question_text" required class="input min-h-[100px]" placeholder="Tuliskan soal di sini... (Bisa langsung Paste / Ctrl+V gambar ke kotak ini)" rows="3" on:paste={(e) => handlePaste(e, 'q-text')}></textarea>
				</div>
