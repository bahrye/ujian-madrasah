<script lang="ts">
	...
	import { exportAllExamTypeResults } from '$lib/utils/excel';
	...
	async function handleExportExcel() {
		...
		const res = await exportAllExamTypeResults(data.examType.id, data.classData.id);
		isExportingExcel = false;
		if (res.success) {
			toasts.success('Rekap nilai berhasil di-export ke Excel!');
			showPrintModal = false;
		} else {
			toasts.error('Gagal export rekap nilai: ' + (res.error || 'Terjadi kesalahan'));
		}
	}
...
<svelte:head>
	<title>{data.classData.name} - {data.examType.name} — Ujian Online</title>
</svelte:head>
						...
						{#if exam.sessions && exam.sessions.length > 0}
							<div class="flex flex-col gap-1 w-full">
								{#each exam.sessions as session}
									<div class="flex items-center justify-between bg-slate-50 p-1.5 rounded border border-slate-100">
										<span class="font-semibold text-slate-700">Sesi {session.session_number}</span>
										<span class="text-slate-600 text-[10px]">
											{session.start_time ? parseDate(session.start_time).toLocaleString('id-ID', { dateStyle: 'short', timeStyle: 'short' }) : '-'}
											s/d
											{session.end_time ? parseDate(session.end_time).toLocaleString('id-ID', { dateStyle: 'short', timeStyle: 'short' }) : '-'}
										</span>
									</div>
								{/each}
							</div>
...
{#if showCreateModal}
	...
	<div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" on:click={() => (showCreateModal = false)}>
		<div class="max-h-[90vh] overflow-y-auto card p-6 w-full max-w-lg animate-bounce-in" on:click|stopPropagation>
			<h2 class="text-lg font-bold text-slate-800 mb-4">Buat Ujian Baru di {data.classData.name}</h2>
			<form method="POST" action="?/create" use:enhance={() => { return async ({ update }) => { showCreateModal = false; await update(); }; }} class="space-y-4">
				...
				<div class="grid grid-cols-2 gap-3 bg-slate-50 p-3 rounded-lg border border-slate-100 relative">
					{#if useSessionsCreate}
						<div class="absolute inset-0 bg-slate-100/60 z-10 rounded-lg flex items-center justify-center backdrop-blur-[1px]">
							<span class="bg-white px-3 py-1 rounded shadow-sm text-xs font-semibold text-slate-500">Waktu default dinonaktifkan</span>
						</div>
					{/if}
					<div>
				...
				<div class="border border-slate-200 rounded-lg p-3">
					...
					<div class="grid grid-cols-2 gap-3 pl-6 border-l-2 border-indigo-100 mt-3" class:hidden={!useSessionsCreate}>
				...
				<div class="flex items-center gap-2">
					...
					<label for="c-active" class="text-sm font-medium text-slate-700">Ujian Langsung Aktif</label>
				</div>
			...
			<form method="POST" action="?/update" use:enhance={() => { return async ({ update }) => { editingExam = null; await update(); }; }} class="space-y-4">
				...
				<div class="grid grid-cols-2 gap-3">
					<div>
						...
						<select id="e-subject" name="subject_id" class="input" required value={editingExam.subject_id}>
							<option value="">Pilih Mata Pelajaran</option>
							{#each data.subjects as subject}
								<option value={subject.id}>{subject.name}</option>
							{/each}
...
{#if showPrintModal}
	...
	<div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" on:click={() => (showPrintModal = false)}>
		<div class="bg-white rounded-xl shadow-xl w-full max-w-md flex flex-col" on:click|stopPropagation>
			<div class="p-6 border-b border-slate-100 flex items-center justify-between">
				<h3 class="text-lg font-bold text-slate-800">Cetak Berkas</h3>
			...
			<div class="p-6 flex flex-col gap-3">
				...
				<button 
					type="button"
					class="btn-outline flex items-center justify-center w-full text-emerald-700 border-emerald-200 hover:bg-emerald-50 disabled:opacity-50 font-semibold mt-1" 
					on:click={handleExportExcel}
					disabled={isExportingExcel}
				>
					{#if isExportingExcel}
						<svg class="animate-spin w-4 h-4 mr-2 text-emerald-600" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
						Mengexport Excel...
					{:else}
						<svg class="w-4 h-4 mr-2 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
