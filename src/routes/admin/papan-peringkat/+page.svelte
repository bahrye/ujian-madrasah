...
<div class="space-y-6 animate-in">
	<div class="mb-8">
		...
		<p class="text-slate-500 mt-1">Lihat peringkat siswa untuk seluruh ujian di sekolah.</p>
	</div>
				...
				{#each data.exams as exam}
					<div class="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm flex flex-col hover:shadow-md transition-shadow">
						...
						<div class="mt-auto pt-5 border-t border-slate-100">
							<a href="/admin/papan-peringkat/{exam.id}" class="btn btn-primary w-full justify-center">
								Lihat Peringkat
							</a>
						</div>
					</div>
			...
			<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
				{#each data.examTypes as type}
					<div class="bg-gradient-to-br from-indigo-500 to-primary-600 rounded-2xl p-6 shadow-md text-white flex flex-col hover:shadow-lg transition-shadow">
						...
						<div class="mt-auto">
							<a href="/admin/papan-peringkat/type/{type.id}" class="bg-white text-primary-600 hover:bg-slate-50 font-semibold px-4 py-2.5 rounded-xl w-full text-center inline-block transition-colors shadow-sm">
