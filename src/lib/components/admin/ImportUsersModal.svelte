<script lang="ts">
  ...
  async function parseExcel() {
    ...
    try {
      ...
      const targetSheetName = workbook.SheetNames.find(name => name === "Data Pengguna") || workbook.SheetNames[0];
      ...
      if (results.length === 0) {
        throw new Error("Tidak ada data valid yang dapat dibaca. Pastikan Username, Nama Lengkap, dan Password terisi.");
      }
      ...
      // Submit form
      setTimeout(() => {
        const submitBtn = document.getElementById('submit-import-users-btn');
  ...
  <div use:portal class="fixed inset-0 z-[100] flex items-center justify-center">
    ...
    <div class="bg-white w-full max-w-lg rounded-2xl shadow-2xl relative z-10 flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
      <div class="p-6 border-b border-slate-100 flex items-center justify-between sticky top-0 bg-white rounded-t-2xl z-10">
        <h2 class="text-xl font-bold text-slate-800">Import Data Pengguna</h2>
      ...
      <div class="p-6 overflow-y-auto">
          ...
          <div class="bg-gradient-to-br from-indigo-50 to-violet-50 border border-indigo-100 rounded-2xl p-5">
            ...
            >
              ...
              <div class="flex-1 min-w-0">
                ...
                <p class="text-indigo-200 text-xs mt-0.5">Format siap pakai dengan Petunjuk Pengisian</p>
              </div>
        ...
        <form method="POST" action="?/importExcel" class="hidden">
          <input type="hidden" name="users_json" value={JSON.stringify(parsedData)} />
          <button type="submit" id="submit-import-users-btn" aria-label="Submit Form"></button>
        </form>
      </div>
